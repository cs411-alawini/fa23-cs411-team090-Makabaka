...
5. DLCPrice: an integer attribute, the price of the DLC.
...
`HaveDLC` is a relation between `GameInfo` and `DLCInfo` to mark the DLCs that each game has. This is a **one-to-many** relation, for each game can have multiple DLCs, but each DLC can only belong to one game.
